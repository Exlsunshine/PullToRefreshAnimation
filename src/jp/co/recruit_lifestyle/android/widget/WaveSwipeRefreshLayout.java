 ...
 * @author amyu
 */
public class WaveSwipeRefreshLayout extends ViewGroup implements
	...
	/**
	 * {@link WaveSwipeRefreshLayout#mAnimateToCorrectPosition }
	 * にセットするInterpolator
	 */
	...
	/**
	 * {@link WaveSwipeRefreshLayout#mListener} に�?知するかどうかのFlag
	 */
...
//		 ViewCompat.setChildrenDrawingOrderEnabled(this, true);
	...
	public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		mTarget.measure(makeMeasureSpecExactly(getMeasuredWidth()
				- (getPaddingLeft() + getPaddingRight())),
				makeMeasureSpecExactly(getMeasuredHeight()
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		mTarget.layout(getPaddingLeft(), getPaddingTop(), childRight,
				childBottom);

		final int circleWidth = mCircleView.getMeasuredWidth();
		final int circleHeight = mCircleView.getMeasuredHeight();
		mCircleView.layout((thisWidth - circleWidth) / 2, -circleHeight,
				(thisWidth + circleWidth) / 2, 0);

		mWaveView.layout(getPaddingLeft(), getPaddingTop(), childRight,
				childBottom);
	}
	...
	private boolean onMoveTouchEvent(@NonNull MotionEvent event,
			int pointerIndex) {
		if (mIsBeingDropped) {
			return false;
		}
		...
		float originalDragPercent = overScrollTop
				/ (DEFAULT_CIRCLE_TARGET * metrics.density);
		...
		float tensionPercent = (4f - tensionSlingshotPercent)
				* tensionSlingshotPercent / 8f;
		...
		if (originalDragPercent < 1f) {
			...
			mCircleView.setProgressStartEndTrim(0f,
					Math.min(MAX_PROGRESS_ROTATION_RATE, strokeStart));
	...
	private void ensureTarget() {
		...
		if (mTarget == null) {
			throw new IllegalStateException(
					"This view must have at least one AbsListView");
		}
	}

	/**
	 * @param refreshing
	 *            Refreshの状�? * @param notify Listenerに�?知するかどう�?
	 */
	...
	/**
	 * @return {@link WaveSwipeRefreshLayout#mState} == REFRESHING of
	 *         {@link WaveSwipeRefreshLayout.STATE}
	 */
	...
	/**
	 * @param refreshing
	 *            {@link WaveSwipeRefreshLayout#mState} のセット
	 */
	...
	/**
	 * @param listener
	 *            {@link WaveSwipeRefreshLayout.OnRefreshListener}
	 */
	...
	/**
	 * Custom view has progress drawable. Some features of
	 * MaterialProgressDrawable are decorated.
	 * 
	 * @author jmatsu
	 */
	private class ProgressAnimationImageView extends AnimationImageView
	{
		...
		{
			super(context);
			mProgress = new MaterialProgressDrawable(context,
					WaveSwipeRefreshLayout.this);

			if (DisplayUtil.isOver600dp(getContext())) { // Make the progress be
															// big
		...
		public void measure() {
			...
			measure(makeMeasureSpecExactly(circleDiameter),
					makeMeasureSpecExactly(circleDiameter));
		}
		...
		public void setProgressColorSchemeColorsFromResource(
				@IdRes int... resources) {
